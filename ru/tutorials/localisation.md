Свое первое приложение я локализовал кодом - проверял язык через `Locale`. Я прокачался и расскажу вам путь мастера. Туториал научит вас как перевести свое приложение, автоматизировать процесс и поправлять профессиональных разработчиков по плюрализации.
...
Найдите секцию `Localizations`. Нажмите на кнопку `+` и выберите нужные языки.
...
// ПОСЛЕ ЭКСПОРТА УБРАТЬ, ПОЯСНИТЬ
Если в локализационной строке встретится два спецификатора одинакового типа - Xcode автоматически пронумерует их после экспорта. 
...
В `strings`-файле это выглядит так:
...
// ДОПИСАТЬ
Если поменяем порядок элементов, то....
//, в котором хотим видеть их в тексте. Например, если создадим `localisedString` так: 
...
/// УПРОСТИТЬ
// УПОМНЯТЬ ЧТО ВСЕ СИСТЕМНОЕ ЗДЕСЬ
// ГДЕ ВЗЯТЬ ФАЙЛ
`InfoPlist` - файл, в котором лежат ключ-пары для идентификации и конфигурации бандла. Их можно и нужно локализовать.
...
// КОММЕНТАРИЙ ПРО ИСПОЛЬЗЛВАНИЕ КАМЕРЫ
На русском:
