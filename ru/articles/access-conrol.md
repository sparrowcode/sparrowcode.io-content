...
`open` схож с `public`. Он разрешает доступ из других модулей. Отличие состоит в том, что это относится исключительно к свойствам и методам внутри класса, а также к самим классам.
...
Приватный `сеттер` используют чтобы не предоставлять доступ к записи свойства за пределами структуры (класса).
