...
В рамках модуля (в любом месте проекта) мы создаем подкласс на его основе.
...
>Классы `WritingTool` и `Pencil` доступны только внутри нашего модуля из-за `internal` уровня.
